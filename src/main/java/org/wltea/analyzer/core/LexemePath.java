...
class LexemePath extends QuickSortSet implements Comparable<LexemePath> {
    ...
    boolean addCrossLexeme(Lexeme lexeme) {
        if (this.isEmpty()) {
            ...
            return true;
            // ydd 是否有相交部分（歧义，举例：的 vs 的确），若相交则返回true，其余情况返回false
        } else if (this.checkCross(lexeme)) {
            this.addLexeme(lexeme);
            if (lexeme.getBegin() + lexeme.getLength() > this.pathEnd) { // ydd 歧义词往后延伸了一些字
                this.pathEnd = lexeme.getBegin() + lexeme.getLength();
            }
    ...
    boolean checkCross(Lexeme lexeme) {
        return (lexeme.getBegin() >= this.pathBegin && lexeme.getBegin() < this.pathEnd) // ydd 另外一种交叉 [中华,华人]，命中条件1
                // ydd 他,说,的确 [pathBegin:2, pathEnd:4] => 然后加入 的[begin:2,end:3,length:1]，命中条件2
